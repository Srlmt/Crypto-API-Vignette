...
```{r Bitcoin image, echo=FALSE}
include_graphics(path="images/Bitcoin.jpg")
```
  
Next, we can focus on the Bitcoin data. We can get daily volume and price data using the `getAggregates` function from the wrapper. We will get the 1 year data ending on "2021-09-30" since that would give us 4 complete quarters. We can then derive variables and generate figures. 
...
We can start by examining the Bitcoin trading volume over the 4 quarters. We can generate a numeric summary. 

```{r Bitcoin volume numeric}
...
We can visualize this data by creating a boxplot. Since we have calculated the day performance (Gain or Loss), we can incorporate that into the plot, with day gain and losses as different colors. 
...
From the chart we can see that from Oct 2020 to Jan 2021, as the volume was increasing, the price was also increasing. We can also see the two tallest red ticks for volume, one in Jan 2021 and another in May 2021 - these were followed by huge price drop.  
...
Ethereum, the second largest cryptocurrency by market cap, is often compared to Bitcoin. As of 2021-10-02, Bitcoin has a market cap of \$900 billion, while Ethereum has a market cap of \$400 billion. We can  first look at the performance of the two cryptocurrencies over the past year. Again, we will grab the 1 year data ending on "2021-09-30" from the API, since that would give us 4 complete quarters. 
...
From the table We can see that in the short term, the performances are similar. However, over 1 year, Ethereum price increased 750%, which is more than twice of Bitcoin's increase of 310%. 
...
Now we have seen the data in the contingency tables, we could also visualize it with bar plots. 
...
From the bar plot, we can see that Bitcoin has much more days of gains than losses in 2020 Q4, but there are more days of losses than gains in 2021 Q2. For Ethereum, the pattern is more consistent, with more days of gains than losses in each quarter. 
...
From the scatterplot, we can see that for many of the quarters, the points are in clusters. This suggests that the price move closely together at least within the quarter. We do see a positive correlation, but we can calculate the correlation coefficient (r) to confirm.
...
With r=0.7443, we can say that the price of Bitcoin and Ethereum have a fairly strong positive linear relationship within the year.
